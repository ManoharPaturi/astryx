...
/**
 * @file useLayoutRegionOwnership.ts
 * @input Layout height mode, content width, and rendered region DOM markers
 * @output Stable refs plus synchronized region ownership and scroll geometry
 * @position Internal Layout hook. This is the single source of truth for
 *   rendered region ownership across direct JSX, RSC/Flight, and transparent
 *   wrappers.
 */
...
function sameDetectedRegion(a: DetectedRegion, b: DetectedRegion): boolean {
  return (
    a.isRegion === b.isRegion &&
    a.hasMixedOwnership === b.hasMixedOwnership &&
    a.usesMiddleScroll === b.usesMiddleScroll &&
    a.label === b.label
  );
}
...
function detectDomRegions(
  ...
  const middleRegions = regions.filter(
    element => element.getAttribute('data-layout-scroll-owner') === 'middle',
  );
  return {
    isRegion: regions.length > 0,
    hasMixedOwnership:
      middleRegions.length > 0 && middleRegions.length < regions.length,
    // A slot is one region. Multiple top-level region roots must agree before
    // the slot opts into shared scrolling; mixed ownership falls back to self.
    usesMiddleScroll:
      regions.length > 0 && middleRegions.length === regions.length,
    label:
      middleRegions
...
export function useLayoutRegionOwnership({
  contentWidth,
  isFill,
}: {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const synchronizeRegionOwnership = () => {
      ...
      const setMixedOwnershipFallback = (
        detected: DetectedRegion,
        regions: Element[],
      ) => {
        if (!detected.hasMixedOwnership) {
          return;
        }
        for (const region of regions) {
          if (region.getAttribute('data-layout-scroll-owner') === 'middle') {
            setOwnedRegionState(region, 'self');
          }
        }
      };
      setMixedOwnershipFallback(next.start, startRegions);
      setMixedOwnershipFallback(next.content, contentRegions);
      setMixedOwnershipFallback(next.end, endRegions);
      ...
      for (const region of [
        ...
        mutationObserver.observe(region, {
          ...
          attributeFilter: [
            'aria-label',
            'data-layout-region',
            'data-layout-scroll-owner',
          ],
        });
      }
    };

    // Observe only slot roots and ownership attributes—not the Layout subtree.
