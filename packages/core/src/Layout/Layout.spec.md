...
- Released default preserved: `yes`; LayoutContent and LayoutPanel still default
  `isScrollable` to true
- Compatibility class: opt-in semantic extension; explicit false in a fill-height
  Layout now joins the middle scrollport, while false in auto height remains
  page/ancestor-owned
- Controlled/uncontrolled behavior: not applicable
- Migration decision: none; existing omitted/true callers retain independent
  region scrolling
...
- The fill-height middle scrollport that coordinates top-level LayoutContent and
  LayoutPanel region roots which opt out of independent scrolling.
...
| region scroll | independent or middle-owned in fill height    | `isScrollable=true` keeps a local scrollport; false participates in Layout's middle scrollport. |
...
| FR7 | In fill height, top-level LayoutContent and LayoutPanel region roots default to independent scrolling; each explicit `isScrollable={false}` slot participates in the focusable middle scrollport while true sibling slots remain pinned. Multiple region roots in one slot must agree; mixed ownership keeps that slot independent. In auto height, no middle scrollport is created. | Current source, docs, tests, and family   | Proposed opt-in behavior; defaults preserved       |
...
- **AV3 — Panel position.** LayoutPanel may occupy logical start or end, or both
  ...
  target.
- **AV4 — Scroll participation.** Direct LayoutContent and LayoutPanel regions
  may keep their default self-scrollport or set `isScrollable={false}` to
  participate in the fill-height middle scrollport. Multiple region roots in
  one slot must agree; mixed ownership keeps the slot independent. Auto-height
  Layouts remain page/ancestor-scrolled.
...
| Two LayoutPanels               | Both instances remain repetitions of the same Panel anatomy part and target.                          | Independent or middle-owned scrolling may be selected per panel.   |
| Mixed scrolling                | False regions move in the middle scrollport; true regions remain pinned and independently scrollable. | Any direct content/start/end subset may opt into middle scrolling. |
...
- Region `isScrollable` defaults to true. In fill height, explicit false opts a
  top-level rendered region root into the middle scrollport; in auto height, the page/ancestor
...
| FR7           | `Layout.test.tsx`, `LayoutSlots.test.tsx`, `contentWidth.test.tsx`, and Storybook browser evidence | Content-only, one-panel, and mixed three-region scrolling in fill/auto height      | Changing defaults, false-region participation, true-region pinning, focusability, or contentWidth containment fails focused structure or browser checks.                          | `audit:Layout/layout`        |
