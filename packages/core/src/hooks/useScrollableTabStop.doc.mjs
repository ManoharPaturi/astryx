...
export const docsDense = {
  description:
    'Keeps tabindex="0" on a scroll container exactly while it can actually be scrolled — content overflows an axis AND that axis is auto/scroll — so keyboard users can reach + scroll it, and a clipping box never becomes a stop the arrow keys cannot answer (WCAG 2.1.1; axe scrollable-region-focusable). Element + direct children watched w/ shared ResizeObserver; attribute written from observer callback — no state, no extra render. Both axes count (incl. RTL inline overflow). Reads layout, so adopting it makes the consuming component client-only.',
  returnDescriptions: {
