...
export interface DefineThemeInput {
   ...
   * here produce CSS.
   *
   * Declared tier bands do not overlap, so at most one matches at any width
   * and no two tiers ever compete. A tier this theme does not declare is not a
   * boundary either: declare `mobile` and `desktop` and desktop covers
   * everything from the phone line up to its own. The theme's own values apply
   * above the highest declared bounded tier unless `wide` is declared.
   *
   * Explicit `tokens` keep their precedence inside a tier. If one pins a token
   * generated by a scale and the tier changes that scale, the pinned step stays
   * fixed while neighbouring generated steps move.
   *
   ...
   * a finger or stylus. A narrow desktop window remains on the fine side.
   *
   * @example
   * ```tsx
   * mobile: {
