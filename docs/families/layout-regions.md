...
| region scroll         | independent, middle-owned, or page-owned where exposed              | `isScrollable=true` gives a region its own scrollport; false joins the fill-height middle scrollport, while auto height remains page-owned | component/Layout-owned |
...
- **FR7 — Scrolling is explicit and composable.** LayoutContent and LayoutPanel
  default `isScrollable` to true, preserving independent region scrollports.
  In a fill-height Layout, each slot whose top-level rendered region is a
  LayoutContent or LayoutPanel with `isScrollable={false}` participates in the
  focusable Layout middle scrollport; independently scrollable sibling slots
  and non-region slot content remain pinned in their own scroll lanes. Multiple
  top-level region roots within one slot must agree on `isScrollable`; mixed
  values conservatively keep that slot independent. In an auto-height Layout,
  false regions remain page/ancestor-owned and no middle scrollport is created. Section and Toolbar do not become scroll containers
...
| Layout / content only                               | Slot presence selects Layout-edge inset                         | Content defaults to an independent scrollport; `isScrollable={false}` opts into the full-width middle scrollport |
| Layout / all regions                                | Start/end stay logical; named regions receive outer/inner inset | Each region independently chooses a pinned self-scrollport or participation in the middle scrollport             |
...
| LayoutContent / independent, middle-, or page-owned | Region's `isScrollable` choice composes with Layout height      | True is independent by default; false joins the fill-height middle scrollport or the auto-height page            |
| LayoutPanel / fixed or resize-driven                | Panel position selects edge treatment                           | A true panel stays pinned in middle-scroll mode; useResizable may replace width ownership                        |
...
| Layout container     | Five-slot region owner with slot contexts                         | `contentWidth` always contains the complete middle composition. In fill height, a slot whose top-level rendered region is a false LayoutContent/LayoutPanel activates the focusable middle scrollport; known arithmetic widths let it span the Layout while preserving the centered frame, and intrinsic or unresolved-variable widths keep that scrollport constrained. |
...
| FR7                  | Layout/LayoutContent/LayoutPanel unit tests plus Storybook mixed-scroll states | Defaults stay independently scrollable; false top-level region roots activate middle scrolling only in fill height; true siblings receive pinned treatment                                                                  | Browser evidence verifies actual scroll motion, independent-region pinning, keyboard focusability, and scrollbar placement across mixed combinations |
