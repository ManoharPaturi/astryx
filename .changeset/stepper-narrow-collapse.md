...
[feat] Stepper collapses itself in narrow containers instead of leaving each consumer to hand-roll a fallback: a horizontal stepper measures its own width and, once a step has under ~112px, drops the labels to a bare track and names the current step in a row beneath it, with a pair of real prev/next controls when `onStepClick` is set. The breakpoint follows the step count rather than the viewport, so a stepper collapses on the width it actually has. `separated` gives up its click targets along with its labels; `on-track` keeps its indicators pressable as nodes on the rail. The full sequence stays intact for screen readers throughout. (#5659)
@ernestt
